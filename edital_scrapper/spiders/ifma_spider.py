...
class IfmaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        editais = response.css(
            'div.tile-list-1 div.video-item.row-fluid div.tileItem'
        )
        ...
        for edital in editais:
            ...
            for i in range(len(attachment_block.css('a::attr(href)'))):
                attachments[i] = {
                    'item': items[i],
                    'link': response.url+attachment_block.css('a::attr(href)')
                                                         .extract()[i]
                }

            yield {
                'title': edital.css('div.tileHeader h2 a::text')
                               .extract_first(),
                'description': edital.css('span::text').extract_first(),
                'attachments': attachments,
                'information': {
                    'date': edital.css('div.tileInfo ul li::text')
                                  .extract()[0],
                    'type': edital.css('div.tileInfo ul li::text')
                                  .extract()[1],
                    'campus': edital.css('div.tileInfo ul li::text')
                                    .extract()[2],
                    'situation': edital.css('div.tileInfo ul li span::text')
                                       .extract_first()
                }
            }
